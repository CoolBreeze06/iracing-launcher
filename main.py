...
# ===== VERSION =====
CURRENT_VERSION = "1.0"
UPDATE_CHECK_URL = "https://raw.githubusercontent.com/YOUR_USERNAME/iracing-launcher/main/version.json"
# ⚠️ À MODIFIER après la création du repo GitHub
...
def show_update_dialog(update_info):
    """
    Affiche le dialog de mise à jour avec 2 options.
...
def handle_update_check():
    """
    Gère tout le processus de vérification et mise à jour.
    Appelé au démarrage de l'application.
    """
    update_info = check_for_updates()
    
    if not update_info:
        return  # Pas de mise à jour ou erreur
    
    # Afficher le dialog de choix
    choice = show_update_dialog(update_info)
    
    if choice == "github":
        # Ouvrir GitHub dans le navigateur
        import webbrowser
        # Extraire l'URL de la page release depuis l'URL de téléchargement
        ...
        # Téléchargement automatique avec fenêtre de progression
        root = tk.Tk()
        root.withdraw()
        ...
        if download_result["filepath"]:
            messagebox.showinfo(
                ...
                f"{download_result['filepath']}\n\n"
                f"Pour installer la mise à jour :\n"
                f"1. Fermez complètement cette application\n"
                f"2. Lancez le nouveau fichier téléchargé\n"
                f"3. Supprimez l'ancien fichier si vous le souhaitez"
            )
        else:
...
def quit_app(icon=None, item=None):
    ...
    remove_lock()  # Supprimer le fichier lock
    if tray_icon:
        tray_icon.stop()
    if window:
        window.destroy()
...
class Api:
    ...
    def check_process_running(self, exe_path):
        ...
        Retourne True si le processus tourne, False sinon
        """
        if not exe_path or not os.path.exists(exe_path):
            return False
    ...
    def get_all_process_statuses(self, apps):
        """
        Retourne le statut de tous les processus en une seule fois
        Plus efficace que d'appeler check_process_running pour chaque app
        """
        statuses = []
        for app in apps:
    ...
    def restart_app(self, app_data):
        """
        Redémarre une application : kill -> wait -> relaunch
        ...
        try:
            ...
            # 1. Tuer le processus s'il tourne
            for proc in psutil.process_iter(['name', 'exe']):
                try:
                    if proc.info['name'] and proc.info['name'].lower() == exe_name:
                        proc.terminate()
            ...
            # 2. Attendre que le processus se termine (max 3 secondes)
            if killed:
                time.sleep(0.5)  # Petit délai pour laisser le process se terminer
    ...
    def minimize_to_tray(self):
        """Minimise la fenêtre dans le tray"""
        hide_window()
...
# Lancer dans un thread pour ne pas bloquer le démarrage
...
def on_closing():
    """Gère la fermeture de la fenêtre avec notification la première fois"""
    settings = load_settings()
    
    # Vérifier si c'est la première fois qu'on ferme
